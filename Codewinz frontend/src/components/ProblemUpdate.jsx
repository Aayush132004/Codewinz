...
function ProblemUpdate() {
   ...
   const {problemId}=useParams();
    ...
    //will fetch these problem initially and when we get problem id as will display its initial data in default values
useEffect(()=>{
const fetch=async()=>{
...
Setproblem(data);
//it resets the fields once the data fetched
reset({
   title:data?.title,
  ...
  referenceSolution:data?.referenceSolution.map((value)=>{
    return{language:value.language,completeCode:value.completeCode};
  }),
  visibleTestCases:data?.visibleTestCases.map((value)=>value),
  hiddenTestCases:data?.hiddenTestCases.map((value)=>value)
...
return (
  <div className="max-w-5xl mx-auto px-4 py-8">
    <h1 className="text-4xl font-bold mb-8 text-center">Update Problem</h1>
